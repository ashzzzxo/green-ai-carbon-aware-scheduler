...
1. The job configuration is loaded.
2. Carbon data is loaded via `CarbonSource`.
3. The scheduler searches over candidate start times and allowed regions to pick:
   - a start time, and  
   - a region with minimum average carbon intensity before the deadline.
4. Baseline vs carbon-aware emission scores are computed.
5. Metrics are appended to `results.csv`.
6. The training script (e.g., `train_model.py`) is launched using `subprocess`.
...
3. Open `notebooks/analysis.ipynb` and run the cells.
